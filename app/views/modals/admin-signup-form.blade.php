...
<div class="modal-content">
    <form role="form" id="adminsignup-form" method="post" action="{{ URL::to('adminsignup') }}">
    	<input type="hidden" name="_token" value="{{ csrf_token() }}">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
			<h4 class="modal-title" id="adminsignupLabel">{{ Lang::get('words.become-member') }}</h4>
		</div>
...
<!--			<div class="form-group form-addon">
		...
		<div class="modal-footer">
			<button type="submit" id="btn-adminsignup" class="btn btn-primary pull-left">{{ Lang::get('words.register') }}</button>
		</div>
	</form>
</div>
</div>
