...
<title>{{ Lang::get('words.admin_alt').' - '.Lang::get('words.albums') }}</title>
@stop
@section('content')
<?php
	...
	<div class="row">
		<div class="col-xs-12">
			<ol class="breadcrumb">
				...
				<li class="active">{{ Lang::get('words.albums') }} (<b>{{ DB::table("albums")->count() }}</b>)</li>
			</ol>
		</div>
	</div>
	<div class="row">
			...
			<div class="col-xs-12">
				<div class="form-group input-group">
					...
					<div class="input-group-btn">
						...
						<a data-target="#add-album-modal" data-toggle="modal" class="btn btn-danger"><span class="fa fa-plus"></span> {{ Lang::get('words.add-new') }}</a>
					</div>
				</div>
			</div>
			...
			<div class="col-lg-3 col-md-4 col-sm-6" style="margin-bottom:15px">
				<div class="panel panel-default">
					...
					<div class="row">
						<div class="col-xs-6">
							<p>
							<a href="{{ URL::to('admin/album/'.$album->slug) }}">
						...
						<div class="col-xs-6">
							...
							<p>{{ Lang::get('words.category') }}:
							@forelse($album->categories as $cat) <a title="{{ $cat->name }} {{ Lang::get('words.albums') }}" class="pjax" href="{{ URL::to('admin/category/'.$cat->slug) }}">{{ $cat->name }}</a><br>
							@empty
                            @endforelse </p>
					...
					<div class="panel-footer" style="padding:10px;">
						<div class="btn-group">
							...
							<a href="{{ URL::to('admin/album/'.$album->slug.'/featured') }}" class="btn btn-sm btn-primary ">@if(!$album->featured) <i class="fa fa-times"></i> @else  <i class="fa fa-check"></i> @endif Featured</a>
						</div>
					</div>
				</div>
			</div>
    	...
    	<div class="modal-content">
    	    <form id="create-album" role="form" method="post" action="{{ URL::to('admin/album/create') }}" enctype="multipart/form-data">
      		...
      		<div class="modal-body">
				...
				<div class="form-group">
					<label>{{ Lang::get('words.album') }} {{ Lang::get('words.artist') }}:</label>
					<input type="text" class="typeahead form-control" data-role="tagsinput" name="artist" placeholder="{{ Lang::get('words.artist') }}">
				</div>
				<div class="form-group">
					<label>{{ Lang::get('words.album') }} {{ Lang::get('words.category') }}:</label>
					<input type="text" class="typeahead2 form-control" data-role="tagsinput" name="category" placeholder="{{ Lang::get('words.category') }}">
				</div>
				<div class="form-group">
					<label>{{ Lang::get('words.album') }} {{ Lang::get('words.language') }}:</label>
					<input type="text" class="typeahead3 form-control" data-role="tagsinput" name="language" placeholder="{{ Lang::get('words.language') }}">
				</div>
				<div class="form-group">
					<label>{{ Lang::get('words.album') }} {{ Lang::get('words.cover') }}:</label>
					<div class="al-art-opt">
