            ...
            <div class="col-md-12">
                <table class="table album-tracks" style="margin-top: 20px;">
            			...
            			@foreach($tracks as $track)
            				...
            				<tr class="scroll-items" data-title="{{ $track->title }}" data-trackid="{{ $track->id }}" data-artist="{{ $track->artist->name }}" data-slug="{{ $track->slug }}" data-track="{{ URL::to('uploads/audios/'.$track->location.'/'.$track->slug.'.mp3') }}" data-cover="{{ URL::to('uploads/albums/'.$album->location.'/thumb/'.$album->slug.'.jpg') }}" data-album_title="{{ $album->title }}" data-waves="{{ URL::to('uploads/audios/'.$album->location.'/wavefiles/'.$track->slug.'.png') }}">
            					<td>
