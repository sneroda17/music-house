...
@section('meta-content')
	...
	<meta property="og:title" content="{{ Lang::get('words.download') }} {{ $track->title }} {{ Lang::get('words.tracks') }}"/>
	<meta property="og:image" content="{{ $albumImg }}"/>
	<meta property="og:description" content="{{ Lang::get('words.download') }} {{ $track->title }} {{ Lang::get('words.tracks') }}"/>
	<title>{{ Lang::get('words.download') }} {{ $track->title }} {{ Lang::get('words.tracks') }}</title>
@stop
...
<div class="row bg-transparent">
	<div class="transparent-img" style="background-image:url('{{ URL::to('uploads/albums/'.$track->album->location.'/thumb/'.$track->album->slug.'.jpg') }}')"></div>
	<div class="album-header">
		<div class="media">
			<div class="media-left play-al-cont hidden-480">
				<img src="{{ URL::to('uploads/albums/'.$track->album->location.'/thumb/'.$track->album->slug.'.jpg') }}">
				<div class="play-album" data-album="{{ $track->album->slug }}">
            		<span class="fa fa-play"></span>
            	</div>
			</div>
			<div class="play-album visible-480" data-album="{{ $track->album->slug }}">
        		<span class="fa fa-play"></span>
        	</div>
			<div class="media-body">
				<h1 class="media-heading">{{ $track->album->title }}</h1>
				<p>
					<span class="text-muted">{{ Lang::get('words.length') }}:</span> {{ Custom::formatMilliseconds($track->album->tracks->sum('duration')) }} ({{ $track->album->tracks->count() }} Tracks) | <span class="text-muted">Released:</span> {{ $track->album->release_date }}
				</p>
				<p>
					<span class="text-muted">{{ Lang::get('words.size') }}:</span> {{ Custom::formatBytes($track->album->tracks->sum('filesize')) }}
				</p>
				<p>
					<span class="text-muted">{{ Lang::get('words.artist') }}:</span> <a class="link-grey" href="{{ URL::to('artist/'.$track->album->artist->slug) }}"> {{ $track->album->artist->name }}</a>
				</p>
				<p>
					<span class="text-muted">{{ Lang::get('words.favorites') }}:</span> <span id="al-fav-count">{{ $track->album->likes() }}</span>
				</p>
				@if($settings->zip_download)<button class="btn btn-labeled btn-primary album-download" data-album="{{ $track->album->slug }}"><span class="btn-label"><i class="fa fa-download"></i></span> {{ Lang::get('words.download') }}</button>@endif
				<div class="album-actions" data-album="{{ $track->album->id }}">
					<button class="btn btn-transparent btn-circle album-like @if($track->album->is_favorite->count()) active @endif"><span class="fa fa-heart"></span></button>
	...
	<div class="table-responsive">
		<table class="table album-tracks">
			...
			<tbody class="scroll-container">
				...
				?>
				<tr class="scroll-items" data-title="{{ $track->title }}" data-trackid="{{ $track->id }}" data-artist="{{ $track->artist->name }}" data-slug="{{ $track->slug }}" data-track="{{ URL::to('uploads/audios/'.$track->location.'/'.$track->slug.'.mp3') }}" data-cover="{{ URL::to('uploads/albums/'.$track->album->location.'/thumb/'.$track->album->slug.'.jpg') }}" data-album_title="{{ $track->album->title }}">
					<td>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
    	<div class="ft-social text-center">
			...
			<a target="_blank" href="https://twitter.com/share?url={{ Request::url() }}&text={{ Lang::get('words.download').' '.$track->title }}&via={{ $settings->twitter_page_id }}" title="" class="btn btn-circle bg-twitter"><span class="fa fa-twitter"></span></a>
			...
			<a target="_blank" href="http://www.linkedin.com/shareArticle?url={{ Request::url() }}&title={{ Lang::get('words.download').' '.$track->title }}" title="" class="btn btn-circle bg-linkedin"><span class="fa fa-linkedin"></span></a>
			<a target="_blank" href="http://digg.com/submit?url={{ Request::url() }}&title={{ Lang::get('words.download').' '.$track->title }}" title="" class="btn btn-circle bg-digg"><span class="fa fa-digg"></span></a>
			<a target="_blank" href="http://www.stumbleupon.com/submit?url={{ Request::url() }}&title={{ Lang::get('words.download').' '.$track->title }}" title="" class="btn btn-circle bg-stumble"><span class="fa fa-stumbleupon"></span></a>
			<a target="_blank" href="https://pinterest.com/pin/create/bookmarklet/?media={{ $albumImg }}&url={{ Request::url() }}&description={{ Lang::get('words.download').' '.$track->title }}" title="" class="btn btn-circle bg-pinterest"><span class="fa fa-pinterest"></span></a>
