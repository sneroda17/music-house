...
// MUSIC DISPLAY
    ...
    $(document).on('click', '.track-play', function(){
        ...
        var _track_wave = that.parents('tr').data('waves');
        ...
        $('.waveformpng').attr('src', _track_wave);
...
// MUSIC LOADER + DECODE
    ...
    $(document).on('click', '.sm2-playlist-bd > li', function(){
        ...
        var trackUrl = href;
        var fileName = trackUrl.substr(trackUrl.lastIndexOf("/")+1);
        var waveFiileName = "wavefiles/"+trackUrl.substr(trackUrl.lastIndexOf("/")+1).replace("mp3","png");
        trackUrl = trackUrl.replace(fileName,waveFiileName);
        $('.waveformpng').attr('src', trackUrl);
        loadMusic(href);

    });
    $(document).on("click","#play-next,.previous",function(){
        /*var href = $(this).find("a").attr("href");
        var trackUrl = href;
        var fileName = trackUrl.substr(trackUrl.lastIndexOf("/")+1);
        var waveFiileName = "wavefiles/"+trackUrl.substr(trackUrl.lastIndexOf("/")+1).replace("mp3","png");
...
*/
    ...
    //Show Login/Registration Modal if user is not logged In

    var url          = 'http://euve255060.serverprofi24.net/public/admin/user';
...
// Signup Form to register users from admin panel.
    $("#adminsignup-form").ajaxForm({
        type: "post",
        delegation: true,
        dataType: "json",
        beforeSubmit: function(){
            $("#btn-adminsignup").attr("disabled", "true");
        },
        success: function(data){
            var n = noty({
                text        : data.message,
            ...
            if(data.status == 'success') {
                ...
                $("#adminsignup").modal('hide');
            }
            $("#btn-adminsignup").removeAttr("disabled");
        }
    });
        ...
        $.post(url,
            ...
            function(data){
                if(data != '') {
                    ...
                    var trackUrl = $('.sm2-playlist-bd li a:first').attr('href');
                    var fileName = trackUrl.substr(trackUrl.lastIndexOf("/")+1);
                    var waveFiileName = "wavefiles/"+trackUrl.substr(trackUrl.lastIndexOf("/")+1).replace("mp3","png");
                    trackUrl = trackUrl.replace(fileName,waveFiileName);
                    loadMusic($('.sm2-playlist-bd li a:first').attr('href'));
               
/**********************************To display waveform png on home***************************************/
                
                    /*track_wave = that.data('waves');
                    png = $('.sm2-playlist-bd li a:first').attr('href');
                    slugfile = png.split('/').pop().split('#')[0].split('?')[0];
                    slug = slugfile.substr(0, slugfile.lastIndexOf('.'));

                    url = track_wave;
                    mystring = slug;
                    ind1 = url .lastIndexOf('/');
